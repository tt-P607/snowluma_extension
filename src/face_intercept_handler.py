...
# 匹配【face:ID】、【face:ID】、【表情：名称】三种标记
_FACE_PATTERN = re.compile(r"【face:(\d+)】|\[face:(\d+)\]|[【\[]表情：([^\]】]+)[】\]]")
...
def _process_text_segment(text: str) -> list[dict[str, Any]]:
    ...
    for match in _FACE_PATTERN.finditer(text):
        # 匹配前的普通文本
...
class FaceInterceptHandler(BaseEventHandler):
    ...
    async def execute(
        ...
        if config is None:
            return EventDecision.SUCCESS, params
        if not cast(SnowLumaExtensionConfig, config).features.enable_react:
            return EventDecision.SUCCESS, params
        ...
        for seg in message_segment:
            ...
            # 检查是否包含表情标记
            if (
                "【face:" not in text_data
                and "[face:" not in text_data
                and "表情：" not in text_data
            ):
                new_segments.append(seg)
                continue
